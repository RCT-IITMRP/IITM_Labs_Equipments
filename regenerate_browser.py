...
<header>
  <div class="header-top">
    ...
    <div class="header-stats">
      ...
      <div class="stat-pill"><span class="num" id="stat-depts">-</span><span class="lbl">IITM Dept / RP Client</span></div>
...
// Helper: find the best "name" field for an equipment entry (dynamic columns)
function eqField(e,keys){for(const k of keys){const v=e[k];if(v)return v;}return '';}
// Shorthand accessors that search common header variations
...
function eqSource(e){return eqField(e,['Source','source']);}

// Compact "index|equipment|lab|department|professor" lines for AI assistant
...
// Populate Department dropdown from lookup sheet
const deptSel=document.getElementById('dept-filter');
filterOptions.departments.forEach(d=>{const o=document.createElement('option');o.value=d;o.textContent=d;deptSel.appendChild(o);});

// Populate Entity Type dropdown from lookup sheet
const entitySel=document.getElementById('entity-filter');
filterOptions.entity_types.forEach(d=>{const o=document.createElement('option');o.value=d;o.textContent=d;entitySel.appendChild(o);});
...
/* CHANGE 2 + CHANGE 3 (JS): getF() updated
   ...
   ● Added 'entityType' — new Entity Type filter
     CHANGE 3 RE-ENABLE: replace  lab:''  with  lab:document.getElementById('lab-filter').value  */
function getF(){return{q:document.getElementById('search-input').value.trim().toLowerCase(),eq:document.getElementById('equip-search-input').value.trim().toLowerCase(),dept:deptSel.value,entityType:entitySel.value,lab:''};}

/* CHANGE 2 (JS): filterData() — added equipment-name-only check using 'eq' */
function filterData(){const{q,eq,dept,entityType,lab}=getF();return equipmentData.filter(e=>{if(dept&&eqDept(e)!==dept)return false;if(entityType&&eqEntityType(e)!==entityType)return false;if(lab&&eqLab(e)!==lab)return false;if(q){const h=Object.values(e).join(' ').toLowerCase();if(!h.includes(q))return false;}if(eq&&!eqName(e).toLowerCase().includes(eq))return false;return true;});}
...
/* Keys to exclude from the generic "all fields" card rendering (shown specially above) */
const CARD_TOP_KEYS_SET=new Set();
/* Detect top display fields dynamically — these are rendered in the card header area */
function cardTopKeys(){return{equip:['Equipment Name','equipment','Name of Equipment','Facilities'],lab:['Lab Name','lab','Name of subordinate laboratories'],dept:['Department','department','Deparment'],prof:['PI','professor','Professor Incharge','Centre'],profEmail:['PI Email','prof_email'],operator:['Operator Incharge','operator'],opEmail:['Operator Mail ID','op_email'],contact:['Contact / Ext No','contact','Contact']};}
function buildTopSet(){const t=cardTopKeys();Object.values(t).forEach(arr=>arr.forEach(k=>CARD_TOP_KEYS_SET.add(k)));}
buildTopSet();

function renderCards(data,q){const w=document.getElementById('card-view');if(!data.length){w.innerHTML='';return;}w.innerHTML=data.map(e=>{const dept=eqDept(e);const cls=deptClass(dept);const pd=eqProf(e)||'';const em=pd.match(/\(([^)]+@[^)]+)\)/);const email=em?em[1].trim():(eqProfEmail(e)||'');const pn=pd.replace(/\s*\([^)]+\)/g,'').trim();const op=eqOperator(e);const opEm=eqOpEmail(e);const ct=eqContact(e);
/* Extra fields: all keys NOT in the top set */
const extraHtml=Object.keys(e).filter(k=>!CARD_TOP_KEYS_SET.has(k)).map(k=>{const v=e[k];if(!v||!v.trim())return'';return`<div class="card-info-row"><svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg><span><span class="label">${esc(k)}:</span> ${hl(v,q)}</span></div>`;}).join('');
return`<div class="eq-card ${cls.card}"><span class="card-dept-tag">${esc(dept||'Unknown')}</span><div class="card-equip-name">${hl(eqName(e),q)}</div><div class="card-lab-name"><svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>${hl(eqLab(e),q)}</div>${(pn||email||op||ct||extraHtml)?'<div class="card-divider"></div>':''}${pn?`<div class="card-info-row"><svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><span><span class="label">PI:</span> ${hl(pn,q)}${email?` &nbsp;<a href="mailto:${esc(email)}" class="contact-link">${esc(email)}</a>`:''}</span></div>`:''}${op?`<div class="card-info-row"><svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><span><span class="label">Operator:</span> ${hl(op,q)}${opEm?` &nbsp;<a href="mailto:${esc(opEm)}" class="contact-link">${esc(opEm)}</a>`:''}</span></div>`:''}${ct&&ct!==' '?`<div class="card-info-row"><svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 11.6 19.79 19.79 0 0 1 1.6 3.08 2 2 0 0 1 3.56 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.69a16 16 0 0 0 5.89 5.89l.9-.9a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg><span><span class="label">Contact:</span> ${esc(ct)}</span></div>`:''}${extraHtml}</div>`;}).join('');}
/* Collect all unique column keys across the entire dataset for table view */
const allColumnKeys=(function(){const s=new Set();equipmentData.forEach(e=>Object.keys(e).forEach(k=>s.add(k)));return [...s];})();

function renderTable(data,q){
const th=document.getElementById('table-head');
const tb=document.getElementById('table-body');
/* Build dynamic headers */
th.innerHTML='<tr>'+allColumnKeys.map((k,i)=>`<th onclick="sortTable(${i})">${esc(k)} <span class="sort-icon">↕</span></th>`).join('')+'</tr>';
tb.innerHTML=data.map(e=>{return'<tr>'+allColumnKeys.map(k=>{const v=e[k]||'';return`<td>${hl(v,q)}</td>`;}).join('')+'</tr>';}).join('');
}
...
function sortTable(col){if(sortCol===col)sortDir*=-1;else{sortCol=col;sortDir=1;}document.querySelectorAll('#table-head th').forEach((th,i)=>{th.classList.toggle('sorted',i===col);const ic=th.querySelector('.sort-icon');if(ic)ic.textContent=i===col?(sortDir===1?'↑':'↓'):'↕';});const key=allColumnKeys[col];currentData.sort((a,b)=>{const av=(a[key]||'').toLowerCase();const bv=(b[key]||'').toLowerCase();return av<bv?-sortDir:av>bv?sortDir:0;});const{q,eq}=getF();renderTable(currentData,q||eq);}
...
document.getElementById('clear-btn').addEventListener('click',()=>{document.getElementById('search-input').value='';document.getElementById('equip-search-input').value='';deptSel.value='';entitySel.value='';render();});
...
function aiAppendResults(matches){
  ...
  matches.forEach(m=>{
    ...
    card.innerHTML=`<div class="ai-result-eq">${esc(eqName(e))}</div><div class="ai-result-lab">${esc(eqLab(e))} · ${esc(eqDept(e))}</div>${m.why?`<div class="ai-result-why">${esc(m.why)}</div>`:''}`;
