...
The "Contact RCT Team" form posts to the same Worker's /contact route, which
...
# "Contact RCT Team" form endpoint — the same Worker's /contact route.
...
<style>
 ...
 #table-view { display: none; }
  ...
  /* "Contact RCT Team" triggers — open the contact form (no address on the page) */
  .contact-rct-btn { display: inline-block; background: var(--gold); color: var(--navy); border: none; border-radius: 999px; padding: 1px 12px; font-family: inherit; font-size: 12px; font-weight: 700; line-height: 1.7; cursor: pointer; vertical-align: baseline; white-space: nowrap; }
  .contact-rct-btn:hover { background: var(--gold-light); }
  .contact-rct-btn:focus-visible { outline: 2px solid white; outline-offset: 2px; }
...
<div id="contact-backdrop">
  <div class="contact-dialog" role="dialog" aria-modal="true" aria-labelledby="contact-title" aria-describedby="contact-desc">
    ...
    <div class="contact-success" id="contact-success" role="status" hidden>
      ...
      <p>The RCT team will get back to you at the email address you provided.</p>
...
/** "Contact RCT Team" form: the Worker's /contact route and the public
...
const CONTACT_LIMITS   = { name: 100, email: 254, company: 150, message: 5000 };
...
if __name__ == '__main__':
    ...
    if CONTACT_PROXY_URL and TURNSTILE_SITE_KEY:
        print(f"\n  Contact RCT Team form posts to: {CONTACT_PROXY_URL}")
    else:
        print("\n  ℹ Contact RCT Team form is disabled until TURNSTILE_SITE_KEY (and")
